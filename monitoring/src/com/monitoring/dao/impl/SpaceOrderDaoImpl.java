...
public class SpaceOrderDaoImpl implements SpaceOrderDao {
	...
	public SpaceOrder getSpaceOrderByUserId(int userId) {
		...
		SpaceOrder SpaceOrder = new SpaceOrder();
		try {
			SpaceOrder = (SpaceOrder) session.createQuery("from SpaceOrder where userId = ?").setParameter(0, userId).uniqueResult();
		} finally {
			session.close();
		}
		return SpaceOrder;
	...
	public int deleteSpaceOrderById(int id) {
		...
		int result = 0;
		try {
			result = session.createQuery("from SpaceOrder where id = ?").setParameter(0, id).executeUpdate();
		} finally {
			session.close();
		}
		return result;
