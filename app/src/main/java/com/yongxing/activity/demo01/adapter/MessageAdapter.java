...
import com.yongxing.activity.demo01.utils.ItemTouchHelperAdapter;
...
public class MessageAdapter extends RecyclerView.Adapter<MessageAdapter.MyViewHolder> implements ItemTouchHelperAdapter {
    ...
    public MessageAdapter(Context context, List<Map<String, Object>> datas, List<String> groupkey) {
    ...
    public int getItemViewType(int position) {
        if (groupkey.contains(mDatas.get(position).get("title"))) {
            return CHECK_TYPE;
        }
        return NORMAL_TYPE;
    }
    ...
    public void onItemDelete(int position) {
        if (mDatas !=  null){
            boolean b = mDatas.contains(mDatas.get(position));
            if (b){
                //删除集合中的对应item的数据
